          ...
          int blocks_per_sample)
{
  ...
  for (i = (blockIdx.x % blocks_per_sample) * blockDim.x + threadIdx.x;
    ...
    cur_weight = cur_weight * weight_map[toffset + i];
...
template<typename T>
__global__ void cunn_SpatialWeightedClassNLLCriterion_sizeAverage_kernel(
          T *output,
          T *total_weight)
{
  if (*total_weight > 0)
          ...
          int blocks_per_sample)
{
  ...
  for (i = (blockIdx.x % blocks_per_sample) * blockDim.x + threadIdx.x;
    ...
    assert(t >= 0 && t < n_classes);
    T cur_weight = weights ? weights[t] : ScalarConvert<int, T>::to(1);
    cur_weight = cur_weight * weight_map[toffset + i];
    gradInput[ioffset + i + map_nelem * t] = -(cur_weight) * norm;
  }
}
