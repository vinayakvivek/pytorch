          ...
          THTensor *total_weight)
{
  ...
  weight_map = THTensor_(newContiguous)(weight_map);
  weights = weights ? THTensor_(newContiguous)(weights) : NULL;

  real *input_data = THTensor_(data)(input);
  THIndex_t *target_data = THIndexTensor_(data)(target);
  real *weight_map_data = THTensor_(data)(weight_map);
  ...
  if (THTensor_(nDimension)(input) == 1) {
    ...
    output_data[0] = -input_data[cur_target] * total_weight_data[0] * weight_map_data[0];
          ...
          THTensor *total_weight)
{
  ...
  if (THTensor_(nDimension)(input) == 1) {
    ...
    real cur_weight = (!sizeAverage && weights) ? -weights_data[cur_target] : -1;
    cur_weight *= weight_map_data[0];

    gradInput_data[cur_target] = cur_weight;
    ...
    for (i = 0; i < batch_size; i++){
      ...
      gradInput_data[i * n_target + cur_target] = -(cur_weight);
